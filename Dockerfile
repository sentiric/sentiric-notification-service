...
RUN CGO_ENABLED=0 GOOS=linux go build \
    ...
    -o /app/bin/sentiric-notification-service ./cmd/notification-service
...
COPY --from=builder /app/bin/sentiric-notification-service .
RUN chown appuser:appgroup ./sentiric-notification-service

# GÜVENLİK: Kullanıcıyı değiştir
USER appuser

ENTRYPOINT ["./sentiric-notification-service"]